...
def train_augm_w2v(data_file, vocab_file, model_name, validation_file, syns_file=None, embedding_size=300, epochs=10, batch_size=10, num_neg_samples=5, learning_rate=0.01, w2v_init=True, w2v_path=None, emb_npy_file=None, data_augmentation_ratio=.25):
    """
    ...
    model_name : str
        name for the trained model, file structure
        to store model and checkpoints will be
        created accordingly
    validation_file : str
        filepath to the lightweight validation
        dataset
    syns_file : str, optional
        filepath to the lightweight augmented
        dataset, if None skips all augmentation
        (default: None)
    ...
    if not os.path.isdir(model_path):
        os.mkdir(model_path)
    model_file = model_path + model_name + '.pth'
    # Checkpoint directory housekeeping
    checkpoints_folder = model_path + 'checkpoints/'
    if not os.path.isdir(checkpoints_folder):
        os.mkdir(checkpoints_folder)
    if not checkpoints_folder.endswith('/'): checkpoints_folder += '/'
    
    if syns_file:
        open_syns = open(syns_file, 'r', encoding='utf-8', errors='replace')
    else:
        print('No synonym file, running unaugmented')
        # open_syns = nullcontext()
        open_syns = dummy_context_mgr()
    ...
    with open(data_file, 'r', encoding='utf-8', errors='replace') as d, \
        ...
        if syns_file: syns = csv.reader(s)
    ...
    losses_file = model_path + 'losses.csv'
    
    with open(losses_file, 'w+') as l:
        writer = csv.writer(l, quoting=csv.QUOTE_NONNUMERIC)
        
        rows = [['epoch', 'train_loss', 'val_loss']]
        
        for epoch in range(epochs):
            row = [epoch, losses[epoch], val_losses[epoch]]
            rows.append(row)
