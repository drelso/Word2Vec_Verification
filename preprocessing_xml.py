...
if __name__ == '__main__':
    data_basedir = 'data/'
    bnc_data_dir = data_basedir + 'bnc_baby/'
    proc_data_dir = data_basedir + 'processed_data/'
    
    clean_data_name = proc_data_dir + 'bnc_baby_proc_data'
    
    dataset_savefile = clean_data_name + '_1.txt'
    tags_savefile = clean_data_name + '_tags_1.txt'
    ...
    ctx_size = 5
    skipgram_filename = proc_data_dir + 'skipgram_data_ctx' + str(ctx_size)
    skipgram_filename_lt = skipgram_filename + '_lt_v' + str(voc_threshold)
    
    proc_train_data_file = skipgram_filename + '_train.csv'
    lt_proc_train_data_file = skipgram_filename_lt + '_train.csv'
    lt_proc_train_data_file_npy = skipgram_filename_lt + '_train.npy'
    
    proc_val_data_file = skipgram_filename + '_val.csv'
    lt_proc_val_data_file = skipgram_filename_lt + '_val.csv'
    lt_proc_val_data_file_npy = skipgram_filename_lt + '_val.npy'
    
    # PROCESS ALL TEXT FILES AND SAVE TO A SINGLE
    # RAW TEXT FILE
    # process_all_datafiles(bnc_data_dir, dataset_savefile, tags_savefile=tags_savefile, use_headwords=False, include_heads=False, replace_nums=True, replace_unclass=True)
    ...
    new_dataset = clean_data_name + '_1.txt'
    dataset_tags = clean_data_name + '_tags_1.txt'
    new_train_data_file = clean_data_name + '_train_1.txt'
    new_train_tags_file = clean_data_name + '_train_tags_1.txt'
    new_val_data_file = clean_data_name + '_val_1.txt'
    new_val_tags_file = clean_data_name + '_val_tags_1.txt'
